...
public class JedisUtils {
	...
	public static JedisPoolConfig createPoolConfig(int maxIdle, int maxTotal,
		...
		poolConfig
				.setTimeBetweenEvictionRunsMillis(checkingIntervalSecs * 1000);
	...
	public static String getAndCheckValue(JedisTemplate jedis, String key,
			Log log, boolean error) {
		String value = jedis.get(key);
		if (key == null) {
			String message = "Value of key:" + key + " is null.";
			if (error) {
				log.error(message);
